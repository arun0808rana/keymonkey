...
function App() {
  ...
  const handleDuplicateKeypresses = (ki) => {
    // // return while intial render
    // if (key === "") return;
    console.log(wordList.charAt(cursor) === key, "whateec----------");
  ...
  const handleKeyDown = (e) => {
    ...
    console.log(e.key, key, "key-------");

    if (e.key === key) {
      // handleDuplicateKeypresses();
      console.log('inside********')
    ...
    <div className="container mx-auto min-h-screen">
      <div className="flex items-center justify-center w-full h-screen">
        <div className="test-container">
          ...
          {wordList.substring(cursor + wrongLetter.length, 300)}
        </div>
      </div>
